...
function getTasksForDay(tasks, durationDays, dayNumber, completedTasks, phaseIdx) {
  ...
  const todayTaskIdx = Math.min(
    Math.floor((dayNumber - 1) / Math.max(daysPerTask, 1)),
    tasks.length - 1
  )
  for (let i = 0; i <= todayTaskIdx; i++) {
    const key = `${phaseIdx}-${i}`
    if (!completedTasks[key]) return [{ task: tasks[i], taskIdx: i }]
  }
  return [{ task: tasks[todayTaskIdx], taskIdx: todayTaskIdx }]
}
...
function getActiveTaskIdx(tasks, durationDays, dayNumber, completedTasks, phaseIdx) {
  ...
  for (let i = 0; i <= todayTaskIdx; i++) {
    const key = `${phaseIdx}-${i}`
    if (!completedTasks[key]) return i
  }
  return todayTaskIdx
}
...
function daysBeforePhase(roadmap, phaseIdx) {
  let total = 0
  for (let i = 0; i < phaseIdx; i++) total += roadmap[i]?.duration_days || 60
  return total
}
...
export default function Journey() {
  ...
  const addProgress = (phaseIdx, taskIdx, amount, max) => {
    ...
    const current = taskProgress[key] || 0
  ...
  const saveReminder = async () => {
    ...
    try {
      ...
      setReminderMsg(`❌ ${e.message || 'Failed'}`)
    } finally {
  ...
  const ProgressTracker = ({ phaseIdx, taskIdx, tracker, taskKey }) => {
    ...
    const current = taskProgress[taskKey] || 0
    const pct = Math.min((current / max) * 100, 100)

    const config = {
      hours: { label: '⏱️ Progress', unit: 'hrs',  buttons: [1, 2, 3] },
      pages: { label: '📖 Pages',    unit: 'pages', buttons: [10, 20, 50] },
      weeks: { label: '📅 Days read', unit: 'days', buttons: [1] },
    }[type]
      ...
      <div className="mt-3 space-y-2">
        ...
        {!done && (
          <div className="flex gap-2 mt-2">
            {config.buttons.map(amt => (
              <button
                key={amt}
                onClick={() => addProgress(phaseIdx, taskIdx, amt, max)}
              ...
              >
                +{amt} {config.unit === 'days' ? 'day' : config.unit === 'pages' ? 'pg' : 'hr'}
              </button>
            ))}
          </div>
        )}
    ...
    <div className="min-h-screen bg-[#1A1A2E] text-white">
      ...
      <div className="bg-gradient-to-r from-purple-900/50 to-pink-900/50 border-b border-purple-900/30 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => navigate('/profile')} className="w-9 h-9 rounded-full bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center font-bold text-sm border-2 border-purple-500 overflow-hidden">
              {(() => {
        ...
        <AnimatePresence>
          {showReminder && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="max-w-2xl mx-auto mt-3 bg-[#16213E] border border-purple-900/40 rounded-xl p-4 overflow-hidden">
              <p className="text-white font-semibold mb-3">🔔 Daily Reminder</p>
              <div className="flex items-center gap-3 flex-wrap">
                <label className="flex items-center gap-2 cursor-pointer">
                  <div onClick={() => setReminderEnabled(!reminderEnabled)} className={`w-11 h-6 rounded-full transition-all relative cursor-pointer ${reminderEnabled ? 'bg-purple-600' : 'bg-gray-700'}`}>
                ...
                <input type="time" value={reminderTime} onChange={e => setReminderTime(e.target.value)} disabled={!reminderEnabled} className="bg-[#1A1A2E] border border-purple-900/50 text-white px-3 py-1.5 rounded-lg text-sm disabled:opacity-40" />
                <button onClick={saveReminder} disabled={reminderSaving} className="px-4 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold rounded-lg transition-all disabled:opacity-50">{reminderSaving ? 'Saving...' : 'Save'}</button>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-3">
        ...
        <div className="bg-[#16213E] border border-purple-900/30 rounded-2xl px-4 py-3">
          ...
          <div className="h-2.5 bg-purple-900/30 rounded-full overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${streakPct}%` }} transition={{ duration: 0.8, ease: 'easeOut' }} className="h-full rounded-full bg-gradient-to-r from-orange-500 to-pink-500" />
          </div>
      ...
      <div className="max-w-2xl mx-auto px-4 mb-4">
        <div className="flex gap-1 bg-[#16213E] rounded-2xl p-1 border border-purple-900/30">
          {[
            ...
            <button key={key} onClick={() => setActiveTab(key)} className={`flex-1 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === key ? 'bg-purple-600 text-white shadow-md' : 'text-gray-400 hover:text-white'}`}>
              {label}
            </button>
          ))}
        </div>
                ...
                {todaySection.todayTasks.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {todaySection.todayTasks.map(({ task, taskIdx }) => {
                      ...
                      const hasTracker = tracker.type !== null
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            {!hasTracker && (
                              <div className={`mt-0.5 w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all ${done ? 'border-green-500 bg-green-500' : 'border-gray-600'}`}>
                                {done && <span className="text-white text-xs">✓</span>}
                              </div>
                            )}
                            <div className="flex-1">
                              <span className={`text-sm leading-relaxed ${done ? 'text-green-400 line-through' : 'text-gray-200'}`}>{task}</span>
                              {hasTracker && (
                                <ProgressTracker
              ...
              {journey.roadmap.map((phase, phaseIdx) => {
                ...
                return (
                  <div key={phaseIdx} className={`rounded-2xl border overflow-hidden transition-all ${
                    ...
                    <button onClick={() => !isLocked && setExpandedPhase(isExpanded ? -1 : phaseIdx)} className="w-full p-5 text-left" disabled={isLocked}>
                      <div className="flex items-center gap-3">
                      ...
                      {!isLocked && (
                        <div className="mt-3 h-2 bg-purple-900/30 rounded-full overflow-hidden">
                          <motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.5 }} className={`h-full rounded-full ${progress === 100 ? 'bg-green-500' : 'bg-gradient-to-r from-purple-600 to-pink-600'}`} />
                        </div>
                      )}
                    </button>
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-purple-900/30 overflow-hidden">
                          <div className="p-4 space-y-2">
                            {phase.tasks.map((task, taskIdx) => {
                              ...
                              return (
                                ...
                                >
                                  {!hasTracker && (
                                    <div className={`mt-0.5 w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${isFuture ? 'border-gray-700' : done ? 'border-green-500 bg-green-500' : 'border-gray-600'}`}>
                                      {done && !isFuture && <span className="text-white text-[10px]">✓</span>}
                                    </div>
                                  )}
                                  <div className="flex-1">
                                    <span className={`text-sm ${isFuture ? 'text-gray-700' : done ? 'text-green-400 line-through' : 'text-gray-300'}`}>{task}</span>
                                    {isActiveToday && !done && <span className="ml-2 text-[10px] bg-purple-600/40 text-purple-300 px-1.5 py-0.5 rounded-full">Today</span>}
                                    {isFuture && <span className="ml-2 text-[10px] text-gray-700">🔒 future</span>}
                                    {hasTracker && !isFuture && (
                                      <ProgressTracker phaseIdx={phaseIdx} taskIdx={taskIdx} tracker={tracker} taskKey={key} />
                                    )}
                                  </div>
                                </motion.div>
                              )
              ...
              <div className="bg-[#16213E] rounded-2xl p-5 border border-purple-900/30">
                ...
                <div className="space-y-3">
                  {(journey.daily_schedule || []).map((item, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }} className="flex items-center gap-4 p-4 bg-[#1A1A2E] rounded-xl border border-purple-900/20">
              ...
              <div className="bg-[#16213E] rounded-2xl p-5 border border-purple-900/30">
                ...
                {badges.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    {badges.map((badge, i) => (
                      <motion.div key={badge.label} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: i * 0.07, type: 'spring' }} className="bg-gradient-to-br from-purple-900/40 to-pink-900/40 border border-purple-700/40 rounded-2xl p-4 text-center">
